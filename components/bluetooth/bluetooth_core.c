...
extern void bluetooth_event_write_cb(void * data);

static uint8_t adv_config_done = 0;

static uint16_t heart_rate_handle_table[HRS_IDX_NB];
static uint16_t battery_handle_table[BTR_IDX_NB];
static uint16_t test_handle_table[IDX_NB];
...
static const uint8_t body_sensor_loc_val[1] = {0x02};
...
static const uint8_t heart_ctrl_point[1] = {0x01};
...
static const esp_gatts_attr_db_t heart_rate_gatt_db[HRS_IDX_NB] =
{
    ...
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&body_sensor_location_uuid, ESP_GATT_PERM_READ_ENCRYPTED|ESP_GATT_PERM_WRITE_ENCRYPTED,
...
// static const uint16_t primary_service_uuid = ESP_GATT_UUID_PRI_SERVICE;
// static const uint16_t character_declaration_uuid = ESP_GATT_UUID_CHAR_DECLARE;
// static const uint16_t character_descriptor_uuid = ESP_GATT_UUID_CHAR_DESCRIPTION;
// static const uint8_t char_prop_notify = ESP_GATT_CHAR_PROP_BIT_NOTIFY;
// static const uint8_t char_prop_read = ESP_GATT_CHAR_PROP_BIT_READ;
// static const uint8_t char_prop_read_write = ESP_GATT_CHAR_PROP_BIT_WRITE|ESP_GATT_CHAR_PROP_BIT_READ;
// static const uint16_t character_descriptor_uuid = ESP_GATT_UUID_CHAR_DESCRIPTION;
...
// /// Battery Descriptor
// static const uint8_t battery_descriptor[] = "Percent";

/// Heart Rate Sensor Service - Heart Rate Measurement Characteristic, notify
// static const uint16_t battery_level_uuid = ESP_GATT_UUID_BATTERY_LEVEL;
// static const uint8_t battery_level_value = 90;




// /// Full HRS Database Description - Used to add attributes into the database
// static const esp_gatts_attr_db_t battery_gatt_db[BTR_IDX_NB] =
// {
//     //Battery Service Declartion
//     [BTR_IDX_SVC]                    =
//     {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&primary_service_uuid, ESP_GATT_PERM_READ,
//       sizeof(uint16_t), sizeof(battery_service_uuid), (uint8_t *)&battery_service_uuid}},

//     // Battery Service Characteristic Declaration
//     [BTR_IDX_CHARACTERISTIC]                    =
//     {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&character_declaration_uuid, ESP_GATT_PERM_READ,
//       sizeof(uint16_t), sizeof(char_prop_notify), (uint8_t *)&char_prop_notify}},

//     // Battery Level Characteristic
//     [BTR_IDX_LEVEL]                    =
//     {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&battery_level_uuid, ESP_GATT_PERM_READ,
//       sizeof(uint16_t), sizeof(battery_level_value), (uint8_t *)&battery_level_value}},

// };

/* Service */
static const uint16_t GATTS_SERVICE_UUID_TEST      = ESP_GATT_UUID_BATTERY_SERVICE_SVC;
static const uint16_t GATTS_CHAR_UUID_TEST_A       = ESP_GATT_UUID_BATTERY_LEVEL;
static const uint16_t GATTS_CHAR_UUID_TEST_B       = 0xFF05;
                                        ...
                                        esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    switch (event) {
        ...
        case ESP_GATTS_WRITE_EVT:
            ...
            ESP_LOGI(GATTS_TABLE_TAG , "Connection Id: 0x%04x \n" , param->write.conn_id);
            ESP_LOGI(GATTS_TABLE_TAG , "Transition Id: 0x%04x \n" , param->write.trans_id);
            ESP_LOGI(GATTS_TABLE_TAG , "Handle: 0x%04x \n" , param->write.handle);
            bluetooth_event_write_cb(param);
            break;
        ...
        case ESP_GATTS_CREAT_ATTR_TAB_EVT: {
            ...
            if (param->create.status == ESP_GATT_OK){
                ...
                }else if(param->add_attr_tab.num_handle == BTR_IDX_NB) {
                    memcpy(battery_handle_table, param->add_attr_tab.handles,
                    sizeof(battery_handle_table));
                    esp_ble_gatts_start_service(battery_handle_table[BTR_IDX_NB]);
                }else if(param->add_attr_tab.num_handle == IDX_NB) {
                    memcpy(test_handle_table, param->add_attr_tab.handles,
                    sizeof(test_handle_table));
                    esp_ble_gatts_start_service(test_handle_table[IDX_NB]);
                }else{
                    ESP_LOGE(GATTS_TABLE_TAG, "Create attribute table abnormally, num_handle (%d) doesn't equal to HRS_IDX_NB(%d) or CURRENT_TIME_IDX_NB(%d) ",
                         param->add_attr_tab.num_handle, HRS_IDX_NB , CURRENT_TIME_IDX_NB);
                }
            }else{
